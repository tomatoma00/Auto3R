...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    uqmodel.eval()
    state_dict = torch.load('ssimruns/scenebest.pth', map_location="cpu")
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if schema.num_views_to_add(iteration+1)>0:
            scene.save(iteration) 
        if num_views > 0:
            # scene.save(iteration) 
            outpath = f"iteration{iteration}renderuq"
            rendersavedir = f"{args.model_path}/{outpath}/renders"
            depthsavedir = f"{args.model_path}/{outpath}/depth"
            testcam = scene.getCandidateCameras()     
            print(f'uq in iteration{iteration}, all {len(testcam)} test cameras')
            os.makedirs(rendersavedir,exist_ok=True)
            os.makedirs(depthsavedir,exist_ok=True)
            os.makedirs(f"{args.model_path}/{outpath}/uqs",exist_ok=True)
            namelist=[]
            B = 64                       # tune this until you fit in memory
            all_names, all_max = [], 0
            for batchidx in range(0, len(testcam), B):
                batch_cams = testcam[batchidx:batchidx+B]
                ...
                for cam in batch_cams:
                    renderother = modified_render(cam, scene.gaussians, pipe, background)
                    img = torch.clamp(renderother["render"], 0.0, 1.0)
                    depthimg = robust_normalize_gpu(renderother["depth"])
                    batch_img.append(img)
                    batch_depth.append(depthimg)
                    
                    
                batch_img = torch.stack(batch_img)      # [B,3,H,W]
                batch_depth = torch.stack(batch_depth).unsqueeze(1)
                b_max,uq_max = qualitycheck(uqmodel, batch_img,batch_depth,
                                    f"{args.model_path}/{outpath}/uqs",
                                    [f"{c.image_name}.png" for c in batch_cams],
                                    batch_size=B,iter=iteration)
                all_names.extend([f"{c.image_name}.png" for c in batch_cams])
                if uq_max > all_max:
                    maxid = batchidx+b_max
                    all_max = uq_max
            namelist = all_names
            print(f"choosed {namelist[maxid]}")
            scale=1.0
            chooseidx = 0
            ...
            with open(f'{args.model_path}/viewnumber.txt','w') as f:
                for iii in range(len(scene.train_idxs)):
                    f.write(f"{scene.train_idxs[iii]}\n")
        ...
        if num_views > 0:
            # in the experiments, we delete this case, just use the "else" case
            viewpoint_cam = viewpoint_stack[len(viewpoint_stack)-1]
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[15_000, 30_000, 45_000, 60_000, 90_000, 120_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[21_000, 26_000, 30_000,59000,60000])
